...
export const createShadow = (mode: PaletteMode): Shadows => {
  const shadowColor = mode === 'light' ? '#2B181A' : '#ffffff';

  const transparent1 = alpha(shadowColor, 0.2);
  const transparent2 = alpha(shadowColor, 0.14);
  const transparent3 = alpha(shadowColor, 0.12);
...
export const createCustomShadow = (
  palette: Pick<Palette, 'primary' | 'secondary' | 'info' | 'success' | 'warning' | 'error'>,
): CustomShadowProps => {
  const transparent = alpha('#2B181A', 0.24);
